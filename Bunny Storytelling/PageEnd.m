...
-(void)initScreen
{
    ...
    CCMenuItemImage *menuAgain=[CCMenuItemImage itemWithNormalImage:[self getFileName:@"pageEnd/againbtn_out"] selectedImage:[self getFileName:@"pageEnd/againbtn_over"] block:^(id sender){
    ...
    CCMenuItemImage *menuMore=[CCMenuItemImage itemWithNormalImage:[self getFileName:@"pageEnd/morebtn_out"] selectedImage:[self getFileName:@"pageEnd/morebtn_over"] block:^(id sender){
        /*
        NSString *url =[NSString stringWithString:@"http://www.synvata.com"];
    ...
    CCMenuItemImage *menuContact=[CCMenuItemImage itemWithNormalImage:[self getFileName:@"pageEnd/contactbtn_out"] selectedImage:[self getFileName:@"pageEnd/contactbtn_over"] block:^(id sender){
        NSString *url =[@"mailto:swu@synvata.com?subject=  &body=  " stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url]];
    }];
...
-(void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ((location.x>290)&&
        ...
        (location.y<300))
    {
        NSString *url =[@"http://www.synvata.com"  stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url]];
    }
    
    //http://itunes.apple.com/us/app/kuqi-guitar-hero/id496487611?ls=1&mt=8
